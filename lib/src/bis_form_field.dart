...
import 'package:bottom_input_system/src/extensions/autovalidatemode_extension.dart';
...
class BisFormField<T> extends FormField<T> {
  ...
  @override
  BisFormFieldState<BisFormField<T>, T> createState() =>
      BisFormFieldState<BisFormField<T>, T>();
}

class BisFormFieldState<F extends BisFormField<T>, T>
    extends FormFieldState<T> {
  String? _customErrorText;
  BisFormBuilderState? _formBuilderState;
  bool _touched = false;
  bool _dirty = false;
  ...
  void initState() {
    ...
    _formBuilderState = BisFormBuilder.of(context);
  ...
  @override
  void didUpdateWidget(covariant BisFormField<T> oldWidget) {
  ...
  bool validate({
    ...
    final fields = _formBuilderState?.fields ??
        <String, BisFormFieldState<BisFormField<dynamic>, dynamic>>{};

    if (!isValid &&
        focusOnInvalid &&
        (formBuilderState?.focusOnInvalid ?? true) &&
        enabled &&
